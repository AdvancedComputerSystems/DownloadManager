...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		if(this.p.multiselect) {
			this.p.colNames.unshift("<input title='Select All' role='checkbox' id='cb_"+this.p.id+"' class='cbox' type='checkbox'/>");
	...
	showHideCol : function(colname,show) {
		return this.each(function() {
			...
			if(fndh===true) {
			    ...
			    /*
			    *FIXED JQGRID BUG ACS
			    */ 
			    $($t).jqGrid("setGridWidth",$t.p.shrinkToFit === true ? $t.p.width: $t.p.tblwidth);
			}
			if( gh )  {
	...
	groupingRender : function (grdata, colspans ) {
		return this.each(function(){
			...
			$.each(grp.groups,function(i,n){
				 ...
				 * MODIFIED BY ACS
				 */
				hid = clid+"_"+$.base64Encode(n.value);
